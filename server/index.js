...
const SUBSCRIPTIONS_FILE = path_1.default.join(process.cwd(), 'subscriptions.json');
const TWITCH_SUBSCRIPTIONS_FILE = path_1.default.join(process.cwd(), 'twitch_subscriptions.json');
const TWITCH_TOKEN_FILE = path_1.default.join(process.cwd(), 'twitch_token.json');
let twitchOAuthToken = null;
let twitchDeviceAuthUrl = null;
let twitchDeviceCode = null;
let twitchCheckInterval = null;
// Load subscriptions and Twitch token from file
function loadSubscriptions() {
    try {
        if (fs_1.default.existsSync(SUBSCRIPTIONS_FILE)) {
            const data = fs_1.default.readFileSync(SUBSCRIPTIONS_FILE, 'utf-8');
            subscriptions = JSON.parse(data);
            console.log(`✓ Loaded ${subscriptions.length} YouTube subscriptions from file`);
        }
        if (fs_1.default.existsSync(TWITCH_SUBSCRIPTIONS_FILE)) {
            const data = fs_1.default.readFileSync(TWITCH_SUBSCRIPTIONS_FILE, 'utf-8');
            twitchSubscriptions = JSON.parse(data);
            console.log(`✓ Loaded ${twitchSubscriptions.length} Twitch subscriptions from file`);
        }
        if (fs_1.default.existsSync(TWITCH_TOKEN_FILE)) {
            const data = fs_1.default.readFileSync(TWITCH_TOKEN_FILE, 'utf-8');
...
function saveSubscriptions() {
    try {
        fs_1.default.writeFileSync(SUBSCRIPTIONS_FILE, JSON.stringify(subscriptions, null, 2), 'utf-8');
        fs_1.default.writeFileSync(TWITCH_SUBSCRIPTIONS_FILE, JSON.stringify(twitchSubscriptions, null, 2), 'utf-8');
        if (twitchOAuthToken) {
            fs_1.default.writeFileSync(TWITCH_TOKEN_FILE, JSON.stringify(twitchOAuthToken, null, 2), 'utf-8');
        }
    }
    catch (error) {
...
async function initiateTwitchDeviceAuth() {
    if (!TWITCH_CLIENT_ID)
        return;
    try {
        const response = await axios_1.default.post('https://id.twitch.tv/oauth2/device', null, {
            params: {
...
async function pollTwitchDeviceAuth() {
    if (!TWITCH_CLIENT_ID || !twitchDeviceCode)
        return false;
    try {
...
async function notifyNewVideo(discordChannelId, video) {
    try {
        if (!video || !client)
            return;
...
async function getTwitchUserInfo(username) {
    try {
        ...
        if (error.response?.status === 401) {
            ...
            if (fs_1.default.existsSync(TWITCH_TOKEN_FILE)) {
                fs_1.default.unlinkSync(TWITCH_TOKEN_FILE);
            }
            return null;
        }
...
async function checkIfTwitchLive(userId) {
    try {
        const accessToken = await getTwitchAccessToken();
        if (!accessToken) {
            console.error('Could not obtain Twitch access token');
...
async function notifyTwitchLive(discordChannelId, stream, displayName) {
    try {
        if (!client)
            return;
        const channel = await client.channels.fetch(discordChannelId);
...
async function initializeBot() {
    if (DISCORD_TOKEN && YOUTUBE_API_KEY && !client) {
        ...
        client.on('clientReady', () => {
            ...
            client?.user?.setActivity('Eating Cookies', { type: discord_js_1.ActivityType.Custom });
        ...
        client.on('messageCreate', async (message) => {
            try {
                ...
                if (command === 'sub') {
                    ...
                    const index = subscriptions.findIndex((s) => s.channelId === youtubeChannelId && s.discordChannelId === message.channelId);
                    if (index === -1) {
                    ...
                    const index = twitchSubscriptions.findIndex((s) => s.twitchUsername === twitchUsername.toLowerCase() && s.discordChannelId === message.channelId);
                    if (index === -1) {
